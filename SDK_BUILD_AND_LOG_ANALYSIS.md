...
- Turn To 버튼은 진단 중에도 활성 상태를 유지하며 다시 누르면 이전 진단을 종료하고 같은 입력값을 수동 재전송
- 방향 버튼은 누르는 동안 `VLK_Move`를 100 ms 간격으로 재전송하고, 버튼을 놓으면 timer를 멈춘 뒤 `VLK_Stop` 호출
- 안전을 위해 `VLK_TurnTo`는 자동 반복하지 않음
...
- Pitch가 한 번에 목표에 도달하지 않으면 Turn To를 사용자가 다시 눌러 수동 재전송하고 각 시도의 Before/After를 비교한다.
